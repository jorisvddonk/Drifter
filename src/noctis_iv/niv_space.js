...
function finish_space() {
	...
	for (var cx=0; cx<64800; cx++) {
		if (p_background[cx] < 0) {
			...
			if (i < 10) {
				console.log("p_background[" + cx + "] <= 0! (" + p_background[cx] + "). Fixing.");
			} else if (i == 10) {
...
function create_thickatmosphere_space() {
	...
	for (c=0; c<r; c++) {
		...
		Acy = ranged_fast_random (178 - 2*Acr) + Acr;
		switch (RANDOM(2)) {
			case 0: Acx = ((10 * secs) / (ranged_fast_random (3600) + 180)) % 360;
...
function create_thinatmosphere_space() {
	...
	for (c=0; c<r; c++) ssmooth (p_background);
	contrast (ranged_fast_random(200) / 900 + 0.6,
...
function volcano () // un krakatoa volcano con Gedeone il gigante coglione.
{
	for (a=0; a<2*M_PI; a+=4*deg) {
		b = gr;
		for (g=Acr/2; g<Acr; g++) {
...
function permanent_storm () // tempesta permanente (una macchia colossale).
{
	for (g=1; g<Acr; g++) {
		for (a=0; a<2*M_PI; a+=4*deg) {
...
function atm_cyclon () // ciclone atmosferico: un'ammasso di nubi a spirale.
{
	b = 0;
	while (Acr>0) {
...
function storm () // tempesta (una grande macchia chiara sull'atmosfera).
{
	for (g=1; g<Acr; g++) {
		for (a=0; a<2*M_PI; a+=4*deg) {
			px = parseInt(Acx + g * cos (a));
