...
function prepare_star(sysinfo)
{
	...
	for (n=0; n<nop; n++)
	{
		...
		nearstar_p_orb_ecc[n]		= 1 -  c_random (nearstar_p_orb_seed[n] + 10*abs(nearstar_p_orb_tilt[n])) / 2000;
		nearstar_p_ray[n]			= c_random (nearstar_p_orb_seed[n]) * 0.001 + 0.01;
		nearstar_p_ring[n]		= zrandom (nearstar_p_ray[n]) * (1 +  c_random (1000) / 100);
		/*
		...
		*/
		if (s_class != 8)
			nearstar_p_type[n] = c_random (planet_types);
		else
		{
			if (c_random(2))
			{
	...
	if (!no_moons)
	{
		for (n=0; n<nop; n++)
		{
			...
			for (c=0; c<t; c++)
			{
				...
				nearstar_p_orb_seed[q]	= (c*c+4) * nearstar_p_ray[n] + zrandom (300 * nearstar_p_ray[n]) / 100;
				nearstar_p_tilt[q]		= zrandom (10*nearstar_p_orb_seed[q]) / 50;
				nearstar_p_orb_tilt[q]	= zrandom (10*nearstar_p_orb_seed[q]) / 500;
				nearstar_p_orb_ecc[q]		= 1 - c_random (nearstar_p_orb_seed[q] + 10*abs(nearstar_p_orb_tilt[q])) / 2000;
				...
				if (r==3 && s < 9)
				{
					if (n > 7)
						r = 7;
					if (s_class && c_random(4))
						r = 5;
					if (s_class == 2 ||s_class == 7 || s_class == 11)
						r = 8;
				}
				if (r==7 && n <= 5)
					r = 1;
				if ((s_class==2||s_class==5|| s_class==7|| s_class==11) && c_random(n))
					r = 7;
