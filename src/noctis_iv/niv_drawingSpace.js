			function displaySphere(){
				...
				function init() {
				    ...
				    var geommat = new THREE.MeshLambertMaterial({map: THREE_texture, wireframe:false});
...
function renderPlanetTexture(buffer, width, height, scale) {
    ...
    var canvas = $("<canvas width=\"" + width*scale + "\" height=\"" + height*scale + "\"></canvas>");
        ...
        for (var x = 0; x < width; x++) {
            ...
            for (var v = 0; v < scale; v++) {
            	for (var u = 0; u < scale; u++) {
            		setPixel(img,(x*scale)+u,(y*scale)+v,colr.r,colr.g,colr.b,255);
            	}
            }
        }
    }
...
function renderPalette() {
    var canvas = $("<canvas width=\"" + 256*2 + "\" height=\"" + 50 + "\"></canvas>");
	    ...
	    finish_r, finish_g, finish_b)
{
	...
	var k = 1.00 / number_of_colors; //float
