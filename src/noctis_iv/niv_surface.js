...
function std_crater (map, cx, cz, r,
		 lim_h, h_factor, h_raiser, align)
{ // Un cratere. A crater.
	var x, z; //int
		...
		for (z = cz-r; z < cz+r; z++) {
			if (x>-1&&z>-1&&x<align&&z<align) {
				...
				if (d <= fr) {
					...
					y += map[align*parseInt(z)+x];
					if (y<0) y = 0;
					if (y>lim_h) y = lim_h;
					map[align*z+x] = parseInt(y);
				}
			}
		}
}
...
function srf_darkline (map, length,
		   x_trend, z_trend, align)
			...
			for (z = cz-r; z < cz+r; z++) {
				if (x>-1&&z>-1&&x<200&&z<200) {
					...
					if (y>=0) {
						...
						if (allowcanyons == 1) {  //regular pit in the middle
						    if (y>127)
						    y = 254 - y;
	                                        }
						if (allowcanyons == 2) {  //volcano!
							if (y>127) {
								y = 254 - (y - (y/1.3)) - (127/1.3);
								if (y<123)
	                                                        ruinschart[200*z+x]=AF6;
							}
						}
						if (allowcanyons == 0) {  //no pit in middle. normal round hill.
							if (y>hmax)
								y = hmax;
						}
...
function create_volcanic_world() {
			...
			//sctype=LAVA;
				n = RANDOM(10);
				if (n>6) { //Huge volcano, only one.
                                  tempx=100; tempy=100; tempb=2;
				...
				while (n>0) { // Multiple smaller volcanoes
						tempx=RANDOM(180)+10;
						tempy=RANDOM(180)+10;
						//tempb=RANDOM(2)+1;
						tempb=2;
               ...
               while (tempn>0){   //Volcano fluid! Yay! :D
               	if (n <= 6) {
